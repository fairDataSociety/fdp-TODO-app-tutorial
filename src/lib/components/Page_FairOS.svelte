<script>
  ...
  import TocFairos from "./TocFairos.svelte";
...
{#if $next[9]}
  <section>
    ...
    <p>
      A complete list of all available endpoints provided by FairOS is listed
      below.
    </p>
    <div class="notice">
      <dl>
              ...
              <dd>
                <ul>
                  {#each Object.entries(data) as [path, json]}
                    {@const method =
                      json.method == "del" ? "delete" : json.method}
                    <li>
                      <a
                        target="_blank"
                        rel="noreferrer"
  ...
  <section>
    ...
    <div class="notice">
      <p>
        In <strong>Part 2</strong>, we have now covered how to use specific
        FairOS REST API endpoints to create FDP portable accounts as well as to
        build a Todo dapp that stores and retrieves personal data.
      </p>
      <br />
      {#if !$singlePageMode}
